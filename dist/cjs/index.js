...
var swagger = async ({
  ...
  const prefixedPath = (0, import_pathe2.join)(appPrefix, path);
  app.get(path, function documentation2(request) {
    const extPrefix = request.headers["x-forwarded-prefix"] ?? "/";
    const relativePath = (0, import_pathe2.join)(extPrefix, prefixedPath, "json");
    ...
    const extPrefix = request.headers["x-forwarded-prefix"] ?? "/";
    const relativePath = (0, import_pathe2.join)(extPrefix, prefixedPath);
        ...
        if (route.method === "ALL") {
          ALLOWED_METHODS.forEach((method) => {
            registerSchemaPath({
              ...
              hook: route.hooks,
              method,
              path: (0, import_pathe2.join)(extPrefix, appPrefix, route.path),
              // @ts-ignore
